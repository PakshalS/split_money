...
const AddExpenseForm = ({ groupId, onClose }) => {
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      await axios.post(`https://split-money-api.onrender.com/groups/${groupId}/add-expense`, {
        name: expenseName,
        amount,
        paidBy,
        splitAmongst,
      ...
      <div className="bg-gray-950 p-6 rounded-md shadow-md w-96 relative">
        ...
        <div className="mb-4">
          ...
          <button
            ...
            className="bg-gray-900 text-white px-4 py-2 rounded hover:text-green-500 transition duration-300 mb-4"
          >
            Split Equally
          </button>
          {members.map((member, index) => (
        ...
        <button onClick={handleAddExpense} className="bg-gray-900 text-white px-4 py-2 rounded hover:text-green-500">Add Expense</button>
      </div>
    </div>
  );
};
